...
它和普通井字棋不太一样：棋盘里有 9 个小棋盘，你下在哪个小格，会决定对手下一步要去哪个小棋盘。项目里做了双人模式、人机模式、计分板、比分保存，以及多档 AI。
...
- 如果目标小棋盘已经下满，对手可以自由选择其他未满小棋盘。
- 在小棋盘里三连，就占领这个小棋盘。
- 小棋盘被赢下后仍然可以继续落子，只要还有空格。它的归属不会改变，但后续落子仍会决定对手下一步要去的小棋盘。
- 在大棋盘上占领三个连成一线的小棋盘，就赢下整局。
- 如果所有小棋盘都下满但没人完成大棋盘三连，就是平局。
...
- 迭代加深 Alpha-Beta，默认最多 4 层。
- 候选落子排序，优先检查大棋盘胜负、挡棋、小棋盘归属和走位风险。
- 单轮搜索内使用置换表缓存，不写入 `localStorage`。
- 使用更强的局面评估，会看大棋盘两连、小棋盘潜力、中心 / 角落价值，以及把对手送到哪个小棋盘。
- 默认有约 1200ms 的时间限制，超时会使用已经完成的最佳结果。

专家 AI 更强，但不是不败 AI。Ultimate Tic-Tac-Toe 的完整搜索空间很大，这里仍然使用时间限制和启发式评估来保持浏览器可用。
...
- 专家 AI 仍然是限时启发式搜索，后续可以继续调候选排序和局面评估。
